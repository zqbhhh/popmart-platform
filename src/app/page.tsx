...
export default function PopmartMarketPro() {
  ...
  const [isMenuOpen, setIsMenuOpen] = useState(false); // 需求 c: 菜单开关
  ...
  const handlePublish = async () => {
    ...
    const getExpiryDate = () => {
      ...
      if (formData.expiryType === 'today') {
        ...
        // 处理自选日期，拼接 23:00 字符串
        return new Date(`${formData.customDate}T23:00:00`);
      }
      return d;
    };
      ...
      <div className="fixed inset-0 bg-white flex items-center justify-center z-50 p-6 overflow-y-auto">
        <div className="max-w-md w-full space-y-6 my-auto">
          ...
          <div className="text-[14px] text-gray-600 leading-relaxed space-y-4 bg-gray-50 p-6 rounded-2xl">
            <p><strong>1. 信息中转站：</strong>本平台仅提供自提码转让与求购的信息发布，不提供任何担保与支付服务。</p>
            <p><strong>2. 风险自担：</strong>交易双方应自行承担风险。建议核实对方真实性。</p>
            <p><strong>3. 到期管理：</strong>所有发布内容均在设定的日期 <strong>23:00</strong> 自动下架。</p>
          </div>
          <button onClick={() => setHasAgreed(true)} className="w-full bg-[#E60012] text-white py-4 rounded-xl font-bold active:scale-95 transition-all shadow-md">我已阅读并同意</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FA] text-gray-900 pb-16">
      ...
      {/* 修改：顶部 Header 整合个人中心 (需求 c) */}
      <header className="bg-white px-6 py-4 shadow-sm flex justify-between items-center max-w-5xl mx-auto relative">
        ...
        <div className="relative" ref={menuRef}>
          <div 
            ...
            className="flex items-center border border-gray-100 rounded-full pl-2 pr-4 py-1.5 shadow-sm bg-gray-50 cursor-pointer hover:bg-gray-100 transition-colors"
          >
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-gray-400">个人中心 ▼</span>
            </div>
          </div>
          ...
          {isMenuOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-50 overflow-hidden">
              <button 
                onClick={() => { setShowOnlyMine(!showOnlyMine); setIsMenuOpen(false); }}
                className="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-blue-50 flex items-center gap-2"
              >
                <span className={showOnlyMine ? "text-[#2D63FF] font-bold" : ""}>
          ...
          <div className="p-8 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <input placeholder="例如：Angry Molly - 心火怒放系列" className="w-full p-3.5 bg-gray-50 border border-gray-100 rounded-lg outline-none focus:border-[#2D63FF] text-sm" value={formData.item} onChange={e=>setFormData({...formData, item:e.target.value})} />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2 col-span-1 md:col-span-2 pt-2">
                <label className="text-xs text-gray-500">自提有效期限 (统一 23:00 自动下架)</label>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold flex items-center gap-2">
                ...
                实时大厅 {showOnlyMine && <span className="text-sm font-normal text-blue-500">(正在管理我的发布)</span>}
            </h2>
          </div>
          ...
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 space-y-3">
            <div className="flex flex-col sm:flex-row gap-3">
                ...
                <input placeholder="🔍 搜门店" className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 outline-none text-sm focus:border-[#2D63FF]" value={searchStore} onChange={e => setSearchStore(e.target.value)} />
                    ...
                    <div key={post.id} className="relative p-5 border border-gray-50 rounded-xl flex flex-col sm:flex-row justify-between items-center gap-4 bg-gray-50/30 hover:bg-white transition-all group">
                      ...
                      <div className="absolute top-3 right-4 flex flex-col items-end gap-1">
                        ...
                        <span className="text-[10px] bg-orange-50 text-orange-600 px-1.5 py-0.5 rounded border border-orange-100 font-medium">
                          到期：{formatExpiryLabel(post.expires_at)}
                        </span>
                      </div>

                      <div className="flex-1 flex gap-4 items-center w-full mt-4 sm:mt-0">
      ...
      <footer className="max-w-4xl mx-auto px-4 text-center text-[10px] text-gray-400">
          <p>© 2026 专注自提交流站 · 统一 23:00 自动下架 · 交易风险自担</p>
      </footer>
    </div>
  );
}
